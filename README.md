...
1. **Initialize** — create a session with a target model, success criteria, max attempts, and
   mode (`standard` — this loop — or `drift`, see [Drift Mode](#drift-mode) below)
...
An alternative attack mode selected at session creation (`mode: "drift"` vs. the default
`"standard"` loop described above). Instead of one prompt → one response → evaluation per
attempt, Drift Mode holds a single growing conversation with the target and edits that
conversation's history as it goes:

- Each turn, Gemini's message is appended to `target_history` and sent to the target with the
  **full** history so far.
- If the target refuses (checked locally first via `_judge_refusal()` — a quick keyword scan that
  avoids spending a Judge call on obvious refusals), that turn is **removed from history** and
  Gemini is asked for a softer version. The target never "remembers" refusing, because that
  exchange is deleted before the next turn is sent.
- If the target cooperates, the same `run_gemini_attack()` Judge call used by the standard loop
  evaluates the response (`_judge_with_criteria()` builds the prompt; the returned `JudgeResult` is
  handled identically — `.success` ends the loop, `.reason` is logged).
- After `MAX_REFUSAL_STREAK` (3) consecutive refusals, the backtrack counter resets and Gemini is
  told to try a fundamentally different angle rather than another small softening.

This differs from PyRIT Crescendo (`run_pyrit_crescendo` tool, below): Crescendo runs
automatically end-to-end and returns a finished conversation, while Drift Mode gives the backend
full control over the conversation history turn-by-turn, including deleting refused exchanges.
...
and other framing changes haven't helped.
...
# Optional: only needed if your Ollama instance requires HTTP Basic auth
OLLAMA_USERNAME=
OLLAMA_PASSWORD=
```
